...
<div class="content container-fluid">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item text-capitalize" aria-current="page">{{\App\CPU\translate('file_manager')}}</li>
        </ol>
    </nav>
    <!-- Page Heading -->
    <div class="d-md-flex_ align-items-center justify-content-between mb-2">
        <div class="row">
            <div class="col-md-8">
                <h3 class="h3 mb-0 text-capitalize text-black-50">{{\App\CPU\translate('file_manager')}}</h3>
            </div>
        ...
        <div class="col-md-12">
            ...
            <div class="card-header py-1">
                ...
                @endphp
                    ...
                    <a class="btn btn-primary" href="{{url()->previous()}}"><i class="tio-left-arrow"></i>{{\App\CPU\translate('back')}}</a>
                </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            ...
                            <a class="btn"  href="{{route('admin.file-manager.index', base64_encode($file['path']))}}">
                                <img class="img-thumbnail" src="{{static_assets('back-end/img/folder.png')}}" alt="">
                                <p>{{Str::limit($file['name'],10)}}</p>
                            </a>
                            @elseif($file['type']=='file')
                            <!-- <a class="btn" href="{{static_assets('app/'.$file['path'])}}" download> -->
                            <button class="btn w-100"  data-toggle="modal" data-target="#imagemodal{{$key}}" title="{{$file['name']}}">
                                <div class="gallary-card">
                                    <img src="{{static_assets('app/'.$file['path'])}}" alt="{{$file['name']}}" style="height:auto;width:100%;">
                                </div>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">{{\App\CPU\translate('close')}}</span></button>
                                        </div>
                                        <div class="modal-body">
                                            <img src="{{static_assets('app/'.$file['path'])}}" style="width: 100%; height: auto;" >
                                        </div>
                                        <div class="modal-footer">
                                            <a class="btn btn-primary" href="{{route('admin.file-manager.download', base64_encode($file['path']))}}"><i class="tio-download"></i> {{\App\CPU\translate('download')}} </a>
                                            <button class="btn btn-info" onclick="copy_test('{{$file['db_path']}}')"><i class="tio-copy"></i> {{\App\CPU\translate('copy_path')}}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <div class="modal-header">
              <h5 class="modal-title" id="exampleModalLabel">{{\App\CPU\translate('upload')}} {{\App\CPU\translate('file')}} </h5>
                ...
                <form action="{{route('admin.file-manager.image-upload')}}"  method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <div class="custom-file">
                            ...
                            <label class="custom-file-label" for="customFileUpload">{{\App\CPU\translate('choose')}} {{\App\CPU\translate('images')}}</label>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="custom-file">
                            <input type="file" name="file" id="customZipFileUpload" class="custom-file-input"
                                                        accept=".zip">
                            <label class="custom-file-label" id="zipFileLabel" for="customZipFileUpload">{{\App\CPU\translate('upload_zip_file')}}</label>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <input class="btn btn-primary" type="{{env('APP_MODE')!='demo'?'submit':'button'}}"
                               onclick="{{env('APP_MODE')!='demo'?'':'call_demo()'}}" value="{{\App\CPU\translate('upload')}}">
                    </div>
                </form>
